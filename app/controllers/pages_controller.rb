class PagesController < ApplicationController
  ...
  def home
    ...
    @art = calculate_display_sizes(@art)
  end
  ...
  def calculate_display_sizes(artworks)
    min_width = 40
  
    widest = artworks.max_by { |artwork| artwork[:width].to_i }
  
    augmented_artworks = artworks.map do |artwork|
      artwork[:display_percentage] = artwork[:width].to_i * 100 / widest[:width].to_i
      artwork[:display_percentage] = min_width if artwork[:display_percentage] < 25
      artwork
    end
